...
const Login = ({ isSliderMode = false }) => {
  ...
  // DEBUGGING: Simple form without AnimatedAuthContainer to test
  return (
    <div style={{ padding: '20px', maxWidth: '400px', margin: '50px auto', backgroundColor: '#f9f9f9', borderRadius: '10px' }}>
      <h1>DEBUG: Login Form</h1>
      <p>This should show both email/password fields AND Google button</p>
      ...
      <SecureForm onSubmit={handleSubmit} validate={validateForm}>
        ...
        <button 
          type="submit" 
          disabled={loading}
          style={{ width: '100%', padding: '12px', marginBottom: '15px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px' }}
        >
          {loading ? 'Signing in...' : 'Sign In'}
        </button>
      </SecureForm>
      ...
      <button 
        ...
        onClick={handleGoogleLogin}
        disabled={loading}
        style={{ width: '100%', padding: '12px', backgroundColor: '#db4437', color: 'white', border: 'none', borderRadius: '5px' }}
      >
        Continue with Google
      </button>
